<!-- <template>
    <div style="width: 100%;height: 100%;">
        <codemirror ref="myCm"
                    ...
                    @paste.native="OnPaste"></codemirror>
    </div>
...
import 'codemirror/mode/sql/autorefresh.js';
...
export default {
    ...
    created() {
        require('codemirror/theme/' + this.cmTheme + '.css');
        try {
            if (!this.editorValue) {
    ...
    data() {
        return {
            ...
            cmOptions: {
                ...
                lineWrapping: true, //CodeMirror 是否应该滚动或换行长行。 默认为（滚动）。false
                lineNumbers: true, //是否在编辑器左侧显示行号。
                autofocus: true,
                showCursorWhenSelecting: true,
                smartIndent: false, //是否使用上下文相关缩进 mode 提供
                autocorrect: true, //指定是否对输入启用自动更正。
